...
//Função a ser utilizada no Schedule que busca no Web Service do CDCF
pelo método SolicitaClientesParaIntegrar os clinetes a serem incluidos/atualizados.
...
User Function IOCDCF( cXml, cError, cWarning, cParams, oFwEai )
	...
	cXmlCDCF   := MemoRead( '\XML\ConsultaCliente.xml' )
	...
	// Roda o execauto para inserção dos dados
	aArrExec := GerArrExec( 'C350015F-B38C-4F21-AE39-C6611E16989A'/*oFwEai:cUuid*/, cXmlCDCF )

	// IA - Roda o execauto do cadastro de clientes
	RunExecAuto( 'C350015F-B38C-4F21-AE39-C6611E16989A'/*oFwEai:cUuid*/, aArrExec)
...
Static Function DeleteCont( cCodCon, cCodigo )
	...
	If AGA->( DbSeek( cSeek := xFilial( 'AGA' ) + 'SU5' + cCodCon ) )

		Do While ! AGA->( Eof() ) .And. cSeek == AGA->( AGA_FILIAL + AGA_ENTIDA + AGA_CODENT )
	...
	If AGB->( DbSeek( cSeek := xFilial( 'AGB' ) + 'SU5' + cCodCon ) )

		Do While ! AGB->( Eof() ) .And. cSeek == AGB->( AGB_FILIAL + AGB_ENTIDA + AGB_CODENT )
	...
	If AO4->( DbSeek( cSeek := xFilial( 'AO4' ) + 'SU5' + xFilial( 'SU5' ) + cCodCon ) )

		Do While ! AO4->( Eof() ) .And. cSeek == AO4->( AO4_FILIAL + AO4_ENTIDA + AO4_CHVREG )
	...
	If SU5->( DbSeek( cSeek := xFilial( 'SU5' ) + cCodCon ) )

		Do While ! SU5->( Eof() ) .And. cSeek == SU5->( U5_FILIAL + U5_CODCONT )
	...
	If AC8->( DbSeek( cSeek := xFilial( 'AC8' ) + cCodCon + 'SA1' + xFilial( 'SA1' ) + cCodigo ) )

		Do While ! AC8->( Eof() ) .And. cSeek == AC8->( AC8_FILIAL+AC8_CODCON+AC8_ENTIDA+AC8_FILENT+AC8_CODENT )
...
Static Function FReadFile(cCamXml)

	Local cCamXml	:= ""
	Local nOpc		:= GETF_LOCALHARD+GETF_NETWORKDRIVE+GETF_LOCALHARD+GETF_NETWORKDRIVE
	Local cDir		:= "c:\temp\"//SuperGetMv("MV_LOCXML",.F.,Space(6))

	cCamXml := cGetFile('Arquivo XML |*.xml ','Selecione Caminho onde estão os Xmls',,/*cDir*/,.F.,/*nOpc*/,.F.)

Return()



Static Function FLoadTXT(cFileXml)
